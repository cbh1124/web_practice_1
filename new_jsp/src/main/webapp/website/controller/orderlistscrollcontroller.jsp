...
<%Login login = (Login)session.getAttribute("login"); // 로그인된 회원 정보%>   
...
<div class="row mt-5">
	...
	<div class="col-md-8 border rounded p-3"> <!-- 주문 상세 -->
		<%ArrayList<Porderdetail> porderdetails = 
			PorderDao.getPorderDao().getPorderdetaillist(porders.get(i).getOrder_num()); // 2. 주문목록 통해 주문상세 %>
		<% for( int j = 0 ; j<porderdetails.size(); j++ ){ %>
		<% 		Product product = ProductDao.getProductDao().getproduct(porderdetails.get(j).getP_num() ); // 3. 주문상세 통해 제품정보 %>
			<p> 주문 제품내역</p> <hr>
			<div class="row">
